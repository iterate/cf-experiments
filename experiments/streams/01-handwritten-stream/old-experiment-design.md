## Key finding: returned streams are write/resolve pairs

capnweb@0.8.0 returned `ReadableStream` values are not wire-one-way. The API shape is still better
than passing an `onEvent()` callback capability into the DO, because the Stream DO does not call a
subscriber-owned app method or interpret an app acknowledgement. But the Cap'n Web transport currently
serializes each returned-stream chunk as a pipe write to a remote `WritableStream`, and the subscriber
side resolves that write promise.

The concrete post-subscribe WebSocket traffic for two events is:
...
The outbound `resolve undefined` frames are not application-level acknowledgements, but they are still
per-chunk subscriber-originated return traffic. This is the key explanation for the benchmark shape:
raw WebSocket fan-out can expose a nice local `ReadableStream` facade while keeping the wire
one-way after `subscribe`, whereas capnweb@0.8.0 returned streams pay per-chunk write/resolve costs.

There is a Cap'n Web escape hatch for the data plane: `transport=capnweb-oneway` uses a
subscriber-owned sink capability instead of returning a `ReadableStream`. The DO stores a duplicated
sink stub, calls `sink.event(event)`, does not await the returned Cap'n Web thenable, and disposes the
ignored result. After setup, the subscriber receives only inbound frames:
...
This works because Cap'n Web only sends a result `pull` when the caller observes the custom thenable
with `await` / `.then()` / `.catch()` / `.finally()`. Disposing the ignored thenable releases the
result from the caller side without asking the callee to send `resolve undefined`.

`stream-kind=capnweb-after-append` applies the same rule to the main `Stream` DO benchmark surface,
using the exact client-main shape: the client opens the Cap'n Web session with a local main object
that implements `afterAppend({ event })`, calls `subscribeAfterAppendVolatile()` once, and then the
Stream DO calls the client's main object after each storage-free append. The DO does not await the
returned thenable, so pure subscribers originate no per-event frames. This is still per-event Cap'n
Web RPC framing, though, and deployed benchmarks show it remains much slower than raw WebSocket.
...
- "documents the concrete Cap'n Web returned-stream pipe frames" asserts the exact frame shape:
  `in ["stream",["pipeline",...["write"],[event]]]` followed by
  `out ["resolve",...,["undefined"]]` for each chunk.
...
- `transport=capnweb-oneway`: Cap'n Web WebSocket RPC plus subscriber sink capability. The DO calls
  `sink.event(event)` without awaiting a result, so pure subscribers originate no per-event frames
  after setup.
...
| Probe Cap'n Web client-main callback fan-out | Returned streams could be blamed for all Cap'n Web cost, hiding the per-event RPC call overhead left after removing `resolve undefined` return traffic | "client-main afterAppend subscriber originates no websocket frames per event" and `/benchmark/audio-chaos?stream-kind=capnweb-after-append` |
