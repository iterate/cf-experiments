...
| Return idempotent retries before durability validation/accounting/fan-out | Retries could throw on irrelevant invalid retry arguments, duplicate events, write debt, or checkpoints | "idempotent append returns the original event and emits once to live subscribers", "does not count idempotent best-effort retries as new unconfirmed writes", and "idempotent retries return before conflicting validation can reject them" |
...
| Treat `maxOffset` as a contiguous committed-history claim | A missing event key could be silently skipped, producing sparse replay history | "fails replay loudly when committed history has a missing event key" |
