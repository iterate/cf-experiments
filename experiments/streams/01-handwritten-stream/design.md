...
| Validate append argument/event input before idempotency or durability handling | Malformed runtime calls could throw incidental property-access errors, hide behind an idempotent retry, or reach later append logic | "rejects malformed append args before reading event or durability", "rejects malformed append events before idempotency or durability handling", and "rejects malformed idempotent retries before reading the idempotency index" |
