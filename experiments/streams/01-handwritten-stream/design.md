...
| Validate append argument/event input before idempotency or durability handling | Malformed runtime calls could throw incidental property-access errors, hide behind an idempotent retry, silently drop unknown top-level fields, silently ignore misspelled append arguments, or reach later append logic | "rejects malformed append args before reading event or durability", "rejects malformed append events before idempotency or durability handling", "rejects unknown top-level append event fields instead of dropping them", "rejects unknown append argument fields before allocating an offset", and "rejects malformed idempotent retries before reading the idempotency index" |
...
| Resolve and validate per-call durability before allocating an offset | Invalid modes, invalid thresholds, unknown option fields, malformed runtime values like `null`, primitive numbers, or missing object `mode`, or wrong string-form checkpoint threshold fallback could partially mutate the stream, silently fall back to settings, checkpoint at the wrong cadence, or fail with incidental errors | "rejects invalid per-call durability modes before allocating an offset", "rejects invalid per-call checkpoint thresholds", "rejects non-integer checkpoint thresholds before allocating an offset", "rejects null per-call durability before allocating an offset", "rejects object durability without a mode before allocating an offset", "rejects unknown durability option fields before allocating an offset", "rejects primitive per-call durability before falling back to stream settings", and "uses stream checkpoint threshold for checkpointed string overrides" |
