...
interface StreamApi {
  ...
  ): Promise<StreamEventInput & { offset: number; createdAt: string }>;
}
...
const defaultWorkerUrl = "https://00-max-throughput-stream.iterate-dev-preview.workers.dev";
const workerUrl = process.env.WORKER_URL ?? defaultWorkerUrl;

describe("Cap'n Web project capability pipelining", () => {
  it("hits the deployed Worker and pipelines project.streams.get(path).append(event)", async () => {
    ...
    const firstInboundIndex = parsedFrames.findIndex((frame) => frame.direction === "in");
    expect(firstInboundIndex).toBeGreaterThan(2);
    ...
    expect(parsedFrames).toMatchObject([
      ...
      {
        direction: "out",
        data: ["release", 2, expect.any(Number)],
      },
    ]);
  }, 30_000);
});
