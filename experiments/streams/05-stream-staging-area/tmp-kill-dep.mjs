...
const CDP="http://localhost:9444", BASE="https://stream-staging-area.iterate-dev-preview.workers.dev";
const path=`/killdep-${Date.now()}`, url=`${BASE}/streams${path}`;
...
async function rpc(){const u=new URL(BASE);u.pathname=`/stream/${encodeURIComponent(path)}`;u.protocol="wss:";const ws=new WebSocket(u.toString());await new Promise((res,rej)=>{ws.addEventListener("open",res);ws.addEventListener("error",rej);});return {r:newWebSocketRpcSession(ws),ws};}
const N=3000;
const t=await newTab(url); const c=cdp(t.webSocketDebuggerUrl); await c.ready; await sleep(4000);
const {r}=await rpc();
console.log(`appending ${N} events to deployed...`);
await r.appendBatch({events:Array.from({length:N},(_,i)=>({type:"events.iterate.com/debug/random-event",payload:{i}}))});
for(let i=0;i<60;i++){await sleep(1000);const f=JSON.parse(await c.ev(FACTS));if(Number(f.events)>=N+2){console.log("written:",JSON.stringify(f));break;}}
const before=JSON.parse(await c.ev(FACTS));
console.log("before kill:",JSON.stringify(before));
const killStart=Date.now();
await r.kill().catch(()=>{});
let res=null;
for(let i=0;i<80;i++){await sleep(150);const v=await c.ev(FACTS);if(v){const f=JSON.parse(v);if(f.recv!=null&&Number(f.recv)>Number(before.recv)){res={ms:Date.now()-killStart,recvBefore:Number(before.recv),recvAfter:Number(f.recv),recvDelta:Number(f.recv)-Number(before.recv)};break;}}}
console.log("DEPLOYED KILL RESULT:",JSON.stringify(res??"no recv change in 12s"));
