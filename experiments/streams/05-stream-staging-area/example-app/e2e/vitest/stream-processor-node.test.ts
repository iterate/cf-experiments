...
import { connectStream } from "../../../src/node/connect.js";
import { createStreamSubscription } from "../../../src/subscription.js";
import { createProcessorRunner, type Snapshot } from "../../../src/processor-runner.js";
// The SAME processor the DO (outbound) and the browser tab (inbound) run.
import { echoExampleProcessor } from "../../../src/processors/examples/echo/implementation.js";
import type { EchoExampleState } from "../../../src/processors/examples/echo/contract.js";
...
describe("node-hosted stream processor (e2e)", () => {
  ...
  e2eIt("reconnects and resumes from its snapshot without reprocessing", async () => {
    ...
    let saved: Snapshot<EchoExampleState> | undefined;
    const storage = { load: () => saved, save: (s: Snapshot<EchoExampleState>) => void (saved = s) };

    // Session 1: process one input, then drop the connection + runner.
    {
      await using connection = await connectStream({ url: toStreamWebSocketUrl(path) });
      const processorRunner = createProcessorRunner({
        processor: echoExampleProcessor, deps: undefined, storage, stream: connection.stream,
      });
      let handle: { unsubscribe(): void } | undefined;
      await using subscription = createStreamSubscription({
