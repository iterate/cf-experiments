...
import { createProcessorRunner, type Snapshot, type ProcessorStream } from "./processor-runner.js";
import { echoExampleProcessor } from "./processors/examples/echo/implementation.js";
import { circuitBreakerProcessor } from "./processors/circuit-breaker/implementation.js";
import { circuitBreakerProcessorContract } from "./processors/circuit-breaker/contract.js";
import { coreProcessor, getAncestorStreamPaths } from "./processors/core/implementation.js";
...
describe("subscription processor (node, in-process)", () => {
  ...
  it("runs echo side effects only after the subscription anchor", async () => {
    ...
    await runner.processEventBatch({
      events: [
        event({ type: "events.iterate.com/echo-example/input-received", offset: 9, payload: {}, createdAtMs: 9_000 }),
        event({ type: "events.iterate.com/echo-example/input-received", offset: 10, payload: {}, createdAtMs: 10_000 }),
        event({ type: "events.iterate.com/echo-example/input-received", offset: 11, payload: {}, createdAtMs: 11_000 }),
      ],
      streamMaxOffset: 11,
    });
...
class BuiltinStreamSim {
  readonly streams = new Map<string, { processorState: StreamPersistedProcessorState; offset: number }>();
  ...
  append(path: string, input: StreamEventInput, createdAtMs = 0): StreamEvent {
    ...
    entry.processorState = {
      ...
      "circuit-breaker": circuitBreakerProcessorContract.stateSchema.parse(circuitBreakerReduction.state),
    };
