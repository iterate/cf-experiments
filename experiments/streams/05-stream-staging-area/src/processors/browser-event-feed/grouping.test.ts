...
import { GROUP_COMPONENT, groupFeedData, INITIAL_FEED_STATE, planFeedOps } from "./grouping.js";
...
describe("event-feed grouping", () => {
  ...
  it("collapses a run of non-specific events of the same type into one group row that it keeps updating", () => {
    ...
    const { ops, endState } = planFeedOps(INITIAL_FEED_STATE, [e1, e2, e3]);
    expect(ops).toEqual([
      { kind: "insert", localIndex: 0, component: GROUP_COMPONENT, firstOffset: 1, lastOffset: 1, eventCount: 1, data: groupFeedData(DEBUG, [e1]) },
      { kind: "update", localIndex: 0, lastOffset: 2, eventCount: 2, data: groupFeedData(DEBUG, [e1, e2]) },
      { kind: "update", localIndex: 0, lastOffset: 3, eventCount: 3, data: groupFeedData(DEBUG, [e1, e2, e3]) },
    ]);
    expect(endState).toEqual({
      open: { localIndex: 0, firstOffset: 1, lastOffset: 3, eventCount: 3, eventType: DEBUG, events: [e1, e2, e3] },
      nextLocalIndex: 1,
    });
  });
  ...
  it("starts a new group row when the event type changes", () => {
    ...
    expect(ops).toEqual([
      { kind: "insert", localIndex: 0, component: GROUP_COMPONENT, firstOffset: 1, lastOffset: 1, eventCount: 1, data: groupFeedData(DEBUG, [e1]) },
      { kind: "insert", localIndex: 1, component: GROUP_COMPONENT, firstOffset: 2, lastOffset: 2, eventCount: 1, data: groupFeedData(other, [e2]) },
      { kind: "insert", localIndex: 2, component: GROUP_COMPONENT, firstOffset: 3, lastOffset: 3, eventCount: 1, data: groupFeedData(DEBUG, [e3]) },
    ]);
    expect(endState).toEqual({
      open: { localIndex: 2, firstOffset: 3, lastOffset: 3, eventCount: 1, eventType: DEBUG, events: [e3] },
      nextLocalIndex: 3,
    });
  });
  ...
  it("closes the open group when a specific-renderer event arrives, then opens a fresh group", () => {
    ...
    expect(ops).toEqual([
      { kind: "insert", localIndex: 0, component: GROUP_COMPONENT, firstOffset: 1, lastOffset: 1, eventCount: 1, data: groupFeedData(DEBUG, [e1]) },
      { kind: "update", localIndex: 0, lastOffset: 2, eventCount: 2, data: groupFeedData(DEBUG, [e1, e2]) },
      { kind: "insert", localIndex: 1, component: "stream.woken", firstOffset: 3, lastOffset: 3, eventCount: 1, data: { events: [e3] } },
      { kind: "insert", localIndex: 2, component: GROUP_COMPONENT, firstOffset: 4, lastOffset: 4, eventCount: 1, data: groupFeedData(DEBUG, [e4]) },
    ]);
    expect(endState).toEqual({
      open: { localIndex: 2, firstOffset: 4, lastOffset: 4, eventCount: 1, eventType: DEBUG, events: [e4] },
      nextLocalIndex: 3,
    });
  });
